...
def _get_host_interfaces_service(host_id):
    return _get_engine_service().hosts_service().host_service(
        host_id).nics_service()


def _update_host_bonds(bonds, attachments, host_id):
    host_service = _get_engine_service().hosts_service().host_service(host_id)
    host_service.setup_networks(
        modified_bonds=bonds, modified_network_attachments=attachments)
...
def _get_lldp_for_host(host_id, with_bonds):
    ...
    nic_list_without_vlan = utils.filter_out_vlan_interfaces(
        nics_service.list())
    ...
    for nic in nic_list:
        if nic.bonding is None:
            ...
            lldps.update({
                nic: _get_lldp_for_bond_slaves(nics_service, nic)
            })
    return lldps
...
def _get_networks_from_attachments(nic_list, attachment_dict):
    ...
    for nic in nic_list:
        ...
        network_dict.update({
            nic: [
                networks_service.network_service(attachment.network.id).get()
...
def _create_new_bonds(bond_dict, attachment_dict, host_id, next_bond_num):
    ...
    for aggregation_id, nic_list in bond_dict.items():
        network_dict = _get_networks_from_attachments(nic_list,
                                                      attachment_dict)
        updated_nic_list = utils.filter_bond_slaves_by_attachments(
            nic_list, network_dict)
        bond = utils.create_bond_definition(updated_nic_list, next_bond_num)
        attachments = utils.create_attachment_definition(
            updated_nic_list, next_bond_num, attachment_dict)

        if bond is not None:
            logging.info(
                'Re-attaching networks: %s to bond %s', ', '.join(
                    network.name for network in utils.create_network_list(
                        updated_nic_list, network_dict)), bond.name)
...
def _get_lldp_for_nic(nics_service, nic):
    try:
        ...
        lldp_list = nic_service.link_layer_discovery_protocol_elements_service(
        ).list()
...
def _get_nic_label_service(host_id, nic_id):
    return _get_host_interfaces_service(host_id).nic_service(
        nic_id).network_labels_service()
...
def _is_label_present_on_host(host_id, label):
    ...
    for nic in nics:
        attached_labels = [
            attached_label.id for attached_label in _get_nic_label_service(
                host_id, nic.id).list()
        ]
...
def _create_label_candidates_and_assign(host_id, nic, lldps):
    label_candidates = utils.create_label_candidates(
        utils.filter_vlan_tag(lldps))
    logging.info('Created label candidates for nic %s: %s', nic.name,
...
def _get_bond_slave_network_attachments(host_id, nic):
    ...
    return nics_service.nic_service(
        nic.id).network_attachments_service().list()
...
def _run_bond_definition_for_host(host_id):
    ...
    _create_new_bonds(bond_dict, attachment_dict, host_id,
                      utils.find_next_bond_num(nic_names))
